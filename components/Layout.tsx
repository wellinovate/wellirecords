...
export const Layout: React.FC<Props> = ({ 
  ...
  const navItems = [
    ...
    { id: AppView.TELEMED, label: 'Telehealth', icon: Video, locked: !hasAccess },
    { id: AppView.INTEGRATION, label: 'Ecosystem', icon: Network, locked: !hasAccess },
    ...
    { id: AppView.METRICS, label: 'Health Metrics', icon: Activity, locked: !hasAccess },
    { id: AppView.UPLOAD, label: 'Upload Data', icon: UploadCloud, locked: false },
    { id: AppView.LIVE_ASSISTANT, label: 'Welli Voice', icon: Mic, locked: !hasAccess },
    { id: AppView.CHAT, label: 'AI Health Chat', icon: MessageSquareText, locked: !hasAccess },
    { id: AppView.FIND_CARE, label: 'Find Care', icon: MapPin, locked: !hasAccess },
    { id: AppView.PORTABILITY, label: 'Portability', icon: ShieldCheck, locked: !hasAccess },
  ];
    ...
    <div className="flex h-screen bg-slate-950 font-sans selection:bg-blue-900/50">
      ...
      <aside className="w-64 bg-slate-900 border-r border-slate-800 hidden md:flex flex-col flex-shrink-0 z-20">
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
            {!isPremium && (
                <div className={`mb-4 border rounded-xl p-4 text-center transition-all ${daysRemaining <= 3 ? 'bg-red-900/20 border-red-500/30' : 'bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border-indigo-500/30'}`}>
                    <div className="flex justify-between items-start mb-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white shadow-lg ${daysRemaining <= 3 ? 'bg-red-500' : 'bg-indigo-500'}`}>
                    ...
                    <button 
                        onClick={onUpgrade}
                        className={`w-full py-2 text-white rounded-lg text-xs font-bold transition-colors ${daysRemaining <= 3 ? 'bg-red-600 hover:bg-red-500' : 'bg-indigo-600 hover:bg-indigo-500'}`}
                    >
            ...
            <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${currentView === AppView.PROFILE ? 'bg-blue-900/30 text-blue-400' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}
            >
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
         ...
         <header className="h-16 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 md:px-8 z-10 sticky top-0">
            ...
            <div className="hidden md:flex items-center gap-4">
                <span className="text-slate-400 text-sm font-medium">{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                {!isPremium && hasAccess && (
                    <div className="px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 rounded-full flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-indigo-400">{daysRemaining} Days Left in Trial</span>
                    </div>
                )}
                {!hasAccess && (
                     <div className="px-3 py-1 bg-red-500/10 border border-red-500/20 rounded-full flex items-center gap-2">
                        <AlertTriangle size={12} className="text-red-500" />
                        <span className="text-xs font-bold text-red-500">Subscription Required</span>
                    </div>
                )}
            </div>
            ...
            <div className="flex items-center gap-4">
                {isPremium && (
                    <div className="hidden sm:flex items-center gap-1.5 bg-gradient-to-r from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 px-3 py-1.5 rounded-full">
                        ...
                        <span className="text-xs font-bold text-yellow-500 uppercase tracking-wide">Premium</span>
                    </div>
                )}
                ...
                >
                    <div className="text-right hidden md:block leading-tight">
                        <p className="text-sm font-bold text-slate-200 group-hover:text-blue-400 transition-colors">Jane Doe</p>
                        <p className="text-[10px] text-slate-500 font-mono tracking-wide">ID: #8921-22A</p>
                    </div>
      ...
      <div className="md:hidden fixed bottom-0 w-full bg-slate-900 border-t border-slate-800 flex justify-between px-6 py-2 z-50 safe-area-pb shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.3)]">
        {[AppView.DASHBOARD, AppView.LIVE_ASSISTANT, AppView.TELEMED, AppView.CHAT].map((view) => {
             ...
             return (
                 <button
                    key={item.id}
                    onClick={() => onChangeView(item.id)}
                    className={`p-2 rounded-xl flex flex-col items-center gap-1 transition-colors relative ${currentView === item.id ? 'text-blue-400 bg-blue-900/20' : 'text-slate-500'}`}
                 >
                    {item.locked && (
         ...
         <button
            onClick={() => onChangeView(AppView.RECORDS)}
             className={`p-2 rounded-xl flex flex-col items-center gap-1 transition-colors ${currentView === AppView.RECORDS ? 'text-blue-400 bg-blue-900/20' : 'text-slate-500'}`}
         >
            <FileText size={24} />
